...
> 注：可以直接跳到 4. 本地make的完整流程，下面的很多部分都是我用来熟悉makefile写法的内容。
...
TOOLPREFIX := riscv64-linux-gnu-  # 交叉编译工具前缀
CC = $(TOOLPREFIX)gcc             # C编译器
AS = $(TOOLPREFIX)gas             # 汇编器
LD = $(TOOLPREFIX)ld              # 链接器
OBJCOPY = $(TOOLPREFIX)objcopy    # 目标文件转换工具
OBJDUMP = $(TOOLPREFIX)objdump    # 反汇编工具
```

### 1.5 编译标志
```
CFLAGS = -Wall -Werror -O -fno-omit-frame-pointer -ggdb -g  # 基础标志
CFLAGS += -MD                     # 自动生成依赖文件
CFLAGS += -mcmodel=medany         # RISC-V内存模型
CFLAGS += -ffreestanding -fno-common -nostdlib -mno-relax  # 独立环境编译
CFLAGS += -I.                     # 包含当前目录
...
CFLAGS += -D QEMU                # 定义QEMU平台宏
```

* `-ffreestanding`不依赖标准库，操作系统需要自包含；
* `-nostdlib`不连接标准 C 库

### 1.6 链接与运行配置
```
LDFLAGS = -z max-page-size=4096   # 页面大小对齐
linker = ./linker/qemu.ld         # 链接器脚本（定义内存布局）

QEMU = qemu-system-riscv64
RUSTSBI = ./bootloader/SBI/sbi-qemu  # 引导程序
QEMUOPTS = -machine virt -kernel $T/kernel -m 32M -nographic
QEMUOPTS += -smp $(CPUS)          # 多核支持（默认为1）
QEMUOPTS += -bios $(RUSTSBI)      # 指定BIOS
QEMUOPTS += -drive file=fs.img,if=none,format=raw,id=x0  # 虚拟磁盘
...
    @if [ ! -d "./target" ]; then mkdir target; fi  # 确保目录存在
    @$(LD) $(LDFLAGS) -T $(linker) -o $T/kernel $(OBJS)  # 链接内核
    @$(OBJDUMP) -S $T/kernel > $T/kernel.asm  # 生成反汇编
    @$(OBJDUMP) -t $T/kernel | sed '1,/SYMBOL TABLE/d; s/ .* / /; /^$$/d' > $T/kernel.sym  # 生成符号表
```
...
```
**为什么需要initcode**：内核启动后，需要运行第一个用户程序。但内核不能直接执行ELF文件，需要一小段**位置无关的纯二进制代码**作为跳板。
转换流程是：`initcode.S`（源码）→ `initcode.o`（目标文件） → `initcode.out`（可执行文件） → `initcode`（二进制文件），这个二进制代码会**直接嵌入内核**，原因是它也是2.1中构建`Kernel`的依赖项之一。
...
```
这里是在自动生成所有系统调用的汇编包装函数，`usys.pl`脚本中有：
```perl
sub entry {
    my $name = shift;
    print ".global $name\n";
...
.global exec
exec:
    li a7, SYS_exec    # SYS_exec是一个常量，比如7
    ecall
    ret
...
.PRECIOUS: %.o
```
**作用**：防止中间文件被自动删除。 默认情况下，Make在构建完成后会删除中间文件（如 `.o` 文件），`.PRECIOUS` 告诉Make不要删除匹配 `%.o` 模式的中间文件。
...
        dd if=/dev/zero of=fs.img bs=512k count=512; \  # 创建空白镜像
        mkfs.vfat -F 32 fs.img; fi  # 格式化为FAT32
    @mount fs.img $(dst)            # 挂载镜像
    @if [ ! -d "$(dst)/bin" ]; then mkdir $(dst)/bin; fi  # 创建/bin
    @cp README $(dst)/README
    @for file in $$( ls $U/_* ); do \   # 把用户程序都复制到镜像中
        cp $$file $(dst)/$${file#$U/_};\    # copy到根目录
        cp $$file $(dst)/bin/$${file#$U/_}; done   # copy到/bin中
    @cp -r riscv64/* $(dst)             # 把架构文件复制到镜像中
    @umount $(dst)
